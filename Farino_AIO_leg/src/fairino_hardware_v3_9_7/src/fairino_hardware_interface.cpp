...
hardware_interface::CallbackReturn FairinoHardwareInterface::on_activate(const rclcpp_lifecycle::State& previous_state)
{
    ...
    */
    ...
    // ServoJ requires servo streaming mode to be started first (see robot.h:
    // "伺服运动开始，配合ServoJ、ServoCart指令使用" — "Servo motion start, used
    // together with ServoJ/ServoCart commands"). Without this call ServoJ in
    // write() is accepted by the SDK but silently ignored by the controller
    // board, so the real robot never moves even though ROS2 reports success.
    errno_t servoret = _ptr_robot->ServoMoveStart();
    if(servoret != 0){
...
hardware_interface::CallbackReturn FairinoHardwareInterface::on_deactivate(const rclcpp_lifecycle::State& previous_state)
{
    ...
    _ptr_robot->ServoMoveEnd();//end servo motion mode, pairs with ServoMoveStart in on_activate
