...
## 2026-08-27 — S22 고정형 Hand-to-Eye 평면 캘리브레이션 기반

- S22를 조립·검사 스테이션의 고정형 카메라로 사용하기 위해 정규화 영상 좌표와
  FR5 Base 평면 XY 사이의 Homography 캘리브레이션 도구를 구현했다.
- 사용자가 TCP를 실제 평면 기준점에 맞추고 S22 영상의 같은 점을 클릭하면
  `image pixel/normalized coordinate + current TCP Base XYZ + tool number`를 한
  샘플로 저장한다. 이 도구는 로봇 이동 명령을 전혀 보내지 않는다.
- 최소 4점, 권장 8점 이상의 데이터를 RANSAC으로 계산하며 XY RMS/최대 오차,
  평면 Z 표준편차, inlier 수와 영상 coverage를 결과 JSON에 기록한다.
- 품질 기준을 통과하지 못한 결과 또는 기준점 convex hull 밖의 외삽 좌표는
  런타임에서 자동으로 무효 처리하도록 설계했다.
