...
def test_fixed_layout_always_exports_25_slots(tmp_path) -> None:
    # This checks configured slot counts, not field-image registration quality.
    # Supply a synthetic reference instead of requiring an untracked camera asset.
    config = json.loads((PROJECT_DIR / "vision_assembly/config/full_board_inspection.json").read_text())
    reference = tmp_path / "layout_reference.png"
    assert cv2.imwrite(str(reference), np.zeros((1266, 1600, 3), np.uint8))
    config["reference_image"] = str(reference)
    config_path = tmp_path / "layout_config.json"
