...
def generate_launch_description():
    ...
    mock_node = Node(
        package="fairino5_v6_moveit2_config", executable="mock_sim.py", name="mock_movej", output="screen",
        arguments=["--listen-unity", "--velocity", "100", "--acceleration", "100", "--preview-seconds", "0.5"],
        remappings=[("/unity/assembly/start", "/mock_db_mvp/internal/assembly/start"),
    ...
    return LaunchDescription([
        ...
        DeclareLaunchArgument("recipe", default_value=f"{sequencer_share}/config/recipes/assembly-r1.yaml"),
        SetEnvironmentVariable("PRODUCTION_DB_DSN", LaunchConfiguration("production_db_dsn")),
        IncludeLaunchDescription(PythonLaunchDescriptionSource(f"{robot_share}/launch/demo.launch.py")),
        ...
        TimerAction(period=LaunchConfiguration("start_delay"), actions=[mock_node]),
        Node(package="assembly_sequencer", executable="sequencer_node", name="assembly_sequencer_mock", output="screen",
             parameters=[{"recipe": LaunchConfiguration("recipe"),
                          "inspection_fail_probability": ParameterValue(LaunchConfiguration("inspection_fail_probability"), value_type=float),
