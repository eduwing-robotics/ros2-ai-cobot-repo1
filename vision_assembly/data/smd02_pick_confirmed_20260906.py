...
record_path=root/'vision_assembly/data/smd02_pick_attempt_20260906.json'
...
if abs(height+52.177)>1e-6 or recipe['grip']['args'][1]!=12 or recipe['tray_pick_open']['args'][1]!=18:
    raise RuntimeError('SMD02 taught recipe mismatch')
record=dict(part='SMD-02',status='checking',operator_alignment_report='operator confirmed aligned and not touching after explicit question; requested proceeding through grasp',physical_alignment_independently_verified=False,physical_grasp_verified=False,part_held_candidate=False,recipe_sha256=hashlib.sha256(recipe_path.read_bytes()).hexdigest(),started_unix=time.time(),stages=[])
...
try:
    ...
    pe,ae=pose_error(start,[-628.978,-149.655,-42.177,180,0,-7.705])
    if pe>.5 or ae>.2:raise RuntimeError('not at operator-inspected corrected pregrasp position')
    ...
    for z in (height+5,height,height+20):
        ...
        route.append(('SMD02Pick',MotionWaypoint('place_final_50mm_vertical',target,True)))
    ...
    n.gripper(18,'SMD02 taught pre-pick opening')
