...
p=json.loads(Path('/tmp/smd03_assembly_fresh_20260906.json').read_text())
...
record_path=root/'vision_assembly/data/smd03_assembly_pregrasp_retry_20260906.json'
if record_path.exists():raise RuntimeError('existing approach record requires recovery review')
record=dict(part='SMD-03',physical_alignment_verified=False,physical_contact_height_verified=False,part_held=False,measurement_timestamp=p['timestamp_unix'],stages=[],status='checking')
...
def fresh():
    ...
    if p.get('instance_index')!=3 or p.get('operator_confirmed_consumed_prefix_count')!=2:raise RuntimeError('wrong physical instance')
...
try:
    ...
    if math.hypot(xyz[0]+627.377,xyz[1]+139.713)>2:raise RuntimeError('outside reviewed SMD03 region')
    ...
    route=[('SMD03',MotionWaypoint('post_grasp_lift_50mm_vertical',tuple(origin),True))]
    for i in range(1,4):route.append(('SMD03',MotionWaypoint('place_combined_xy_abc_midpoint',tuple(origin[k]+(target[k]-origin[k])*i/3 for k in range(6)),False)))
    ...
    for z in (xyz[2]+100,xyz[2]+50,xyz[2]+20,final_z):route.append(('SMD03',MotionWaypoint('place_final_50mm_vertical',tuple([*xy,z,*target[3:]]),True)))
    ...
    n.gripper(18,'SMD03 inspection opening')
