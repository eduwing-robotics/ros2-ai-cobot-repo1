...
p=json.loads((root/'vision_assembly/data/smd05_operator_target_revalidated_20260906.json').read_text())
...
record_path=root/'vision_assembly/data/smd05_assembly_pregrasp_retry_20260906.json'
if record_path.exists():raise RuntimeError('existing approach record requires recovery review')
record=dict(part='SMD-05',physical_alignment_verified=False,physical_contact_height_verified=False,part_held=False,measurement_timestamp=p['timestamp_unix'],target_source='operator_clicked_terminal_centers_revalidated_against_current_scene',stages=[],status='checking')
def save():record_path.write_text(json.dumps(record,indent=2)+'\n')
def fresh():
    if not (p.get('mode')=='operator_confirmed_terminal_target' and p.get('operator_terminal_points_confirmed') is True and p.get('current_scene_validation',{}).get('automatic_validation_passed') is True) or not 0<=time.time()-p['timestamp_unix']<=120:raise RuntimeError('measurement invalid/expired')
    if p.get('instance_index')!=5 or p.get('operator_confirmed_consumed_prefix_count')!=4:raise RuntimeError('wrong physical instance')
...
try:
    ...
    if math.hypot(xyz[0]+627.422,xyz[1]+114.436)>2:raise RuntimeError('outside reviewed SMD05 region')
    ...
    route=[('SMD05',MotionWaypoint('post_grasp_lift_50mm_vertical',tuple(origin),True))]
    for i in range(1,4):route.append(('SMD05',MotionWaypoint('place_combined_xy_abc_midpoint',tuple(origin[k]+(target[k]-origin[k])*i/3 for k in range(6)),False)))
    ...
    for z in (xyz[2]+100,xyz[2]+50,xyz[2]+20,final_z):route.append(('SMD05',MotionWaypoint('place_final_50mm_vertical',tuple([*xy,z,*target[3:]]),True)))
    ...
    n.gripper(18,'SMD05 inspection opening')
