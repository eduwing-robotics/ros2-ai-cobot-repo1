"""Place operator-confirmed SMD02 from PlaceCamera; no pick or batch execution."""
import argparse
import json
import math
import time
...
def main():
    ...
    parser.add_argument('--confirm-held-smd02', action='store_true')
    args = parser.parse_args()
    if args.execute and not args.confirm_held_smd02:
        parser.error('operator-confirmed SMD02 required')
    if args.record.exists():
        raise RuntimeError('record already exists; no repeated release allowed')
    import hashlib
    root=Path('/home/juchan-yoon/FR5_robot_control')
    held_path=root/'vision_assembly/data/smd02_pick_attempt_20260906.json'
    held=json.loads(held_path.read_text())
    if held.get('physical_grasp_verified') is not True or held.get('placement_completed'):
        raise RuntimeError('SMD02 confirmed held state required')
    ...
    target = board['resolved_placements']['CAP-02']
    if target.get('placement_ready') is not True or target['part_type'] != 'right_white_brown':
        raise RuntimeError('SMD02 placement blocked')
    ...
    node = Executor()
    started = False
    record = {'slot_code': 'CAP-02', 'board_file': str(args.board.resolve()),
    ...
    try:
        ...
        if not (90 < final[0] < 115 and -545 < final[1] < -520 and 80 < final[2] < 95):
            raise RuntimeError('SMD02 target outside reviewed local envelope')
        def at(z): return tuple([*final[:2], z, *final[3:]])
        route = [('CAP-02', MotionWaypoint(label, tcp, linear)) for label,tcp,linear in [
        ...
        for waypoint in checked:
            ...
            if waypoint.label == 'place_final_50mm_vertical':
                node.gripper(17, 'SMD02 release after operator-confirmed grasp')
        ...
        atomic_write(args.record, record)
        held.update(placement_completed=True,placement_record=str(args.record),part_held_candidate=False,status='released_at_CAP02_and_retreated',placement_completed_unix=time.time())
        atomic_write(held_path,held)
        print('SMD02 RELEASE AND RETREAT COMPLETED; PHYSICAL SEATING NOT YET VERIFIED', flush=True)
