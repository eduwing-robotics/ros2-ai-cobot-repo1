"""Place operator-confirmed HBM02 from PlaceCamera; no pick or batch execution."""
import argparse
import json
import math
import time
...
def main():
    ...
    parser.add_argument('--confirm-held-hbm02', action='store_true')
    args = parser.parse_args()
    if args.execute and not args.confirm_held_hbm02:
        parser.error('operator-confirmed HBM02 required')
    ...
    target = board['resolved_placements']['HBM-02']
    if target.get('placement_ready') is not True or target['part_type'] != 'hbm':
        raise RuntimeError('HBM02 placement blocked')
    policy = target['placement_orientation']
    if policy['gripper_axis'] != 'tool_y' or policy['symmetry_period_deg'] != 180:
        raise RuntimeError('unexpected holding-axis policy')
    rclpy.init()
    node = Executor()
    started = False
    record = {'slot_code': 'HBM-02', 'board_file': str(args.board.resolve()),
    ...
    try:
        ...
        if not (50 < final[0] < 75 and -540 < final[1] < -515 and 85 < final[2] < 97):
            raise RuntimeError('HBM02 target outside reviewed local envelope')
        def at(z): return tuple([*final[:2], z, *final[3:]])
        route = [('HBM-02', MotionWaypoint(label, tcp, linear)) for label,tcp,linear in [
        ...
        for waypoint in checked:
            ...
            if waypoint.label == 'place_final_50mm_vertical':
                node.gripper(25, 'HBM02 release after operator-confirmed grasp')
        ...
        atomic_write(args.record, record)
        print('HBM02 RELEASE AND RETREAT COMPLETED; PHYSICAL SEATING NOT YET VERIFIED', flush=True)
