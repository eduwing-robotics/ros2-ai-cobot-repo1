...
TERMINAL = {'motion_complete_awaiting_physical_verification', 'check_completed', 'check_failed', 'recovery_required', 'failed_before_motion', 'failed_recovered'}
...
def main():
    ...
    mode.add_argument('--recover', action='store_true', help='현재 완료/실패 실행을 정리하고 새 ID 테스트를 준비 (무동작)')
    parser.add_argument('--confirm-scene-ready', action='store_true', help='빈 그리퍼·빈 기판·트레이 부품·고정구 정리를 직접 확인')
    parser.add_argument('--operator-id', help='복구 현장 확인 작업자')
    ...
    if args.recover and (not args.confirm_scene_ready or not args.operator_id):
        parser.error("--recover requires --confirm-scene-ready and --operator-id")
    import rclpy
    ...
    try:
        ...
        if args.recover:
            ...
            if not operation_id:
                raise RuntimeError('복구할 실행이 없습니다. 새 ID로 시작하세요.')
            production = bool(status.get('execution_context'))
            request = dict(schema='fr5.assembly_execution/v2' if production else 'fr5.assembly_cycle/v1',
                action='assembly.recover', **{'execution_id' if production else 'operation_id': operation_id},
                scene_confirmation=dict(operator_id=args.operator_id, execution_id=operation_id,
        ...
        correlation_id = request.get('execution_id') or request['operation_id']
        receipt=root/'runtime/assembly_api_clients'/f"{correlation_id}-{uuid4().hex[:8]}.json"
        receipt.parent.mkdir(parents=True,exist_ok=True)
        receipt.write_text(json.dumps(request,indent=2)+'\n')
        updates.clear()
        sent=True
        publish(request)
        print(f"API 요청: {request['action']} / {correlation_id}\n진행 기록: {receipt}",flush=True)
        ...
        while time.monotonic()<deadline:
            ...
            for event in updates[:]:
                ...
                if args.recover:
                    if (event.get('execution_id') or event.get('operation_id')) != correlation_id:
                        continue
                    if event.get('status') == 'request_rejected':
                        print(json.dumps(event, ensure_ascii=False), flush=True)
                        return 1
                    if not event.get('recovery_applied'):
                        continue
                    print(json.dumps(event, ensure_ascii=False), flush=True)
                    return 0
                elif event.get('operation_id')!=request['operation_id'] or event.get('job_id')!=request['job_id']:
                    continue
