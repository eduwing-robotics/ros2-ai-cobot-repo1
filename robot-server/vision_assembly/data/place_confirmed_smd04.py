"""Place operator-confirmed SMD04 from PlaceCamera; no pick or batch execution."""
import argparse
import json
import math
import time
...
def main():
    ...
    parser.add_argument('--confirm-held-smd04', action='store_true')
    args = parser.parse_args()
    if args.execute and not args.confirm_held_smd04:
        parser.error('operator-confirmed SMD04 required')
    if args.record.exists():
        raise RuntimeError('record already exists; no repeated release allowed')
    import hashlib
    root=Path('/home/juchan-yoon/FR5_robot_control')
    held_path=root/'vision_assembly/data/smd04_pick_attempt_20260906.json'
    held=json.loads(held_path.read_text())
    if held.get('physical_grasp_verified') is not True or held.get('placement_completed'):
        raise RuntimeError('SMD04 confirmed held state required')
    ...
    target = board['resolved_placements']['CAP-04']
    if target.get('placement_ready') is not True or target['part_type'] != 'right_white_brown':
        raise RuntimeError('SMD04 placement blocked')
    ...
    node = Executor()
    started = False
    record = {'slot_code': 'CAP-04', 'board_file': str(args.board.resolve()),
    ...
    try:
        ...
        if not (90 < final[0] < 115 and -513 < final[1] < -488 and 80 < final[2] < 95):
            raise RuntimeError('SMD04 target outside reviewed local envelope')
        def at(z): return tuple([*final[:2], z, *final[3:]])
        route = [('CAP-04', MotionWaypoint(label, tcp, linear)) for label,tcp,linear in [
        ...
        for waypoint in checked:
            ...
            if waypoint.label == 'place_final_50mm_vertical':
                node.gripper(17, 'SMD04 release after operator-confirmed grasp')
        ...
        atomic_write(args.record, record)
        held.update(placement_completed=True,placement_record=str(args.record),part_held_candidate=False,status='released_at_CAP04_and_retreated',placement_completed_unix=time.time())
        atomic_write(held_path,held)
        print('SMD04 RELEASE AND RETREAT COMPLETED; PHYSICAL SEATING NOT YET VERIFIED', flush=True)
