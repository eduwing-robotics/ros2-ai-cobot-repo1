...
record_path=root/'vision_assembly/data/smd05_pick_attempt_20260906.json'
...
if abs(height+52.177)>1e-6 or recipe['grip']['args'][1]!=12 or recipe['tray_pick_open']['args'][1]!=18:
    raise RuntimeError('SMD05 taught recipe mismatch')
approach=json.loads((root/'vision_assembly/data/smd05_assembly_pregrasp_retry_20260906.json').read_text())
if approach['status']!='at_pregrasp_alignment_and_contact_height_unverified' or not 0<=time.time()-approach['completed_unix']<60:raise RuntimeError('fresh completed SMD05 approach required')
record=dict(part='SMD-05',status='checking',operator_alignment_report='operator clicked SMD05 terminal centers; fresh scene revalidated against manual target; fixed height exercised by SMD01-04',physical_alignment_independently_verified=False,physical_grasp_verified=False,part_held_candidate=False,recipe_sha256=hashlib.sha256(recipe_path.read_bytes()).hexdigest(),started_unix=time.time(),stages=[])
...
try:
    ...
    for z in (height+5,height,height+20):
        ...
        route.append(('SMD05Pick',MotionWaypoint('place_final_50mm_vertical',target,True)))
    ...
    n.gripper(18,'SMD05 taught pre-pick opening')
