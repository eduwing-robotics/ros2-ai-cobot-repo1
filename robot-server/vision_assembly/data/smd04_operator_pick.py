...
record_path=root/'vision_assembly/data/smd04_pick_attempt_20260906.json'
...
if abs(height+52.177)>1e-6 or recipe['grip']['args'][1]!=12 or recipe['tray_pick_open']['args'][1]!=18:
    raise RuntimeError('SMD04 taught recipe mismatch')
approach=json.loads((root/'vision_assembly/data/smd04_assembly_pregrasp_retry_20260906.json').read_text())
if approach['status']!='at_pregrasp_alignment_and_contact_height_unverified' or not 0<=time.time()-approach['completed_unix']<60:raise RuntimeError('fresh completed SMD04 approach required')
record=dict(part='SMD-04',status='checking',operator_alignment_report='operator clicked both current SMD04 terminal centers; fixture height exercised by SMD01-03',physical_alignment_independently_verified=False,physical_grasp_verified=False,part_held_candidate=False,recipe_sha256=hashlib.sha256(recipe_path.read_bytes()).hexdigest(),started_unix=time.time(),stages=[])
...
try:
    ...
    for z in (height+5,height,height+20):
        ...
        route.append(('SMD04Pick',MotionWaypoint('place_final_50mm_vertical',target,True)))
    ...
    n.gripper(18,'SMD04 taught pre-pick opening')
