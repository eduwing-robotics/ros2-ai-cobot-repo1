...
p=json.loads((root/'vision_assembly/data/smd05_operator_target_final_20260906.json').read_text())
...
record=dict(part='SMD-05',physical_alignment_verified=False,physical_contact_height_verified=False,part_held=False,measurement_timestamp=p['timestamp_unix'],target_source='operator_clicked_terminal_centers',automatic_detection_validation_passed=False,stages=[],status='checking')
def save():record_path.write_text(json.dumps(record,indent=2)+'\n')
def fresh():
    if not (p.get('mode')=='operator_confirmed_terminal_target' and p.get('operator_terminal_points_confirmed') is True) or not 0<=time.time()-p['timestamp_unix']<=120:raise RuntimeError('measurement invalid/expired')
