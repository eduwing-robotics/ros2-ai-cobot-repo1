...
p=json.loads((root/'vision_assembly/data/smd04_operator_target_20260906.json').read_text())
...
record_path=root/'vision_assembly/data/smd04_assembly_pregrasp_retry_20260906.json'
if record_path.exists():raise RuntimeError('existing approach record requires recovery review')
record=dict(part='SMD-04',physical_alignment_verified=False,physical_contact_height_verified=False,part_held=False,measurement_timestamp=p['timestamp_unix'],target_source='operator_clicked_terminal_centers',automatic_detection_validation_passed=False,stages=[],status='checking')
...
def fresh():
    ...
    if p.get('instance_index')!=4 or p.get('operator_confirmed_consumed_prefix_count')!=3:raise RuntimeError('wrong physical instance')
...
try:
    ...
    if math.hypot(xyz[0]+627.696,xyz[1]+127.241)>2:raise RuntimeError('outside reviewed SMD04 region')
    ...
    route=[('SMD04',MotionWaypoint('post_grasp_lift_50mm_vertical',tuple(origin),True))]
    for i in range(1,4):route.append(('SMD04',MotionWaypoint('place_combined_xy_abc_midpoint',tuple(origin[k]+(target[k]-origin[k])*i/3 for k in range(6)),False)))
    ...
    for z in (xyz[2]+100,xyz[2]+50,xyz[2]+20,final_z):route.append(('SMD04',MotionWaypoint('place_final_50mm_vertical',tuple([*xy,z,*target[3:]]),True)))
    ...
    n.gripper(18,'SMD04 inspection opening')
