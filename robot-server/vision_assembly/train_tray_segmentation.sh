...
"${repo_dir}/.venv-vision/bin/python" "${script_dir}/scripts/prepare_tray_segmentation_dataset.py" \
  --val-frames 4 --test-frames 0
exec "${repo_dir}/.venv-vision/bin/yolo" segment train \
  model="${script_dir}/models/tray_segmentation/pilot_02/weights/best.pt" \
  ...
  project="${script_dir}/models/tray_segmentation" name=pilot_03 exist_ok=true
