...
def main():
 ...
 # TrayHome orientation is a section-level placeholder; diagnostic only.
 # Fine angle already passed two-white-terminal and temporal-median gates.
 angle_error=abs((angle-coarse_angle+90.0)%180.0-90.0)
 out={'schema_version':1,'mode':'smd_trayhome_xyz_plus_close_angle','timestamp_unix':now,
  ...
  'long_axis_angle_base_deg':round(angle,3),'coarse_fine_angle_error_deg':round(angle_error,3),'coarse_angle_is_diagnostic_only':True,'position_source':'TrayHome full-tray detection',
  'angle_source':'SMD close-view three-batch two-white-terminal axis','coarse_file':str(a.coarse),'fine_file':str(a.fine)}
