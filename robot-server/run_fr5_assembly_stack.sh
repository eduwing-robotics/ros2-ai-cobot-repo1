...
endpoint_root="${KSMC_UNITY_ENDPOINT_ROOT:-${root}/../main-server/Ros2UnityEndopoint_PKG}"
...
start_stack() {
  [[ -x "${endpoint_root}/run.sh" ]] || {
    echo "Unity Endpoint launcher not found: ${endpoint_root}/run.sh" >&2
...
preflight_stack() {
  ...
  for item in "${endpoint_root}/run.sh" \
