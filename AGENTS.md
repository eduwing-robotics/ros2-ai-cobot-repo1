...
- 각 계층은 인접한 하위 계층의 공개 진입점만 사용하며, 상위 계층이 저장소나 설비 구현을 직접 호출하지 않는다.
- DB, 메시지 전달 방식과 구체 프로세스 배치는 변경 가능한 구현 세부사항이며, 변경 후에도 계층별 책임과 실제 완료·실패 전달 경계를 유지한다.

## 생산 Job·Unit·Recipe 계약
...
- 현행 기준 문서는 프로젝트 정책, 장기간 유지되는 설계와 구현된 public API만 다룬다. 설치·실행 절차는 공개 진입점을 소유한 컴포넌트 README 한 곳에서만 관리한다.
- 시스템 책임과 불변 조건은 `docs/architecture/`가, 컴포넌트 책임은 각 README가, 구체 식별자·payload·오류 의미는 제공 컴포넌트의 API 문서와 소스가 소유한다.
- API 문서에 미구현 인터페이스, 예약 이름, 내부 queue·클래스 구조와 미래 계획을 계약처럼 기록하지 않는다.
- Real·Mock 표기는 구현, 공개 API 또는 완료 감지 방식이 실제로 갈리는 요소에만 사용한다. Job·Unit·DB·레시피와 상위 업무 의미에는 모드 표기를 반복하지 않는다.
- `DATA_STATION/DB`는 MainServer와 Sequencer가 공유하는 production 데이터 계약이며 별도 실행 계층이나 backend로 표현하지 않는다.
- 시점별 상태, TODO, 조사와 과거 계획은 현행 설계에서 분리한다. 보존이 필요하면 `reports/` 또는 `archive/`에 두고 기준 문서나 탐색 목차에 포함하지 않는다.
