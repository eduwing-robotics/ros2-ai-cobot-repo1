...
fairino_msgs__msg__RobotNonrtState__are_equal(const fairino_msgs__msg__RobotNonrtState * lhs, const fairino_msgs__msg__RobotNonrtState * rhs)
{
  ...
  for (size_t i = 0; i < 10; ++i) {
    if (lhs->exaxistatus1[i] != rhs->exaxistatus1[i]) {
      return false;
    }
  }
  // exaxispos2
  ...
  // exaxistatus2
  for (size_t i = 0; i < 10; ++i) {
    if (lhs->exaxistatus2[i] != rhs->exaxistatus2[i]) {
      return false;
    }
  }
  // exaxispos3
  ...
  // exaxistatus3
  for (size_t i = 0; i < 10; ++i) {
    if (lhs->exaxistatus3[i] != rhs->exaxistatus3[i]) {
      return false;
    }
  }
  // exaxispos4
  ...
  // exaxistatus4
  for (size_t i = 0; i < 10; ++i) {
    if (lhs->exaxistatus4[i] != rhs->exaxistatus4[i]) {
      return false;
    }
  }
  // ft_fx_data
  ...
  // auxservoservoid
  if (lhs->auxservoservoid != rhs->auxservoservoid) {
    return false;
  }
  // auxservoerrcode
  if (lhs->auxservoerrcode != rhs->auxservoerrcode) {
    return false;
  }
  // auxservostate
  if (lhs->auxservostate != rhs->auxservostate) {
    return false;
  }
  // auxservoactualpos
  if (lhs->auxservoactualpos != rhs->auxservoactualpos) {
    return false;
  }
  // auxservoctualspeed
  if (lhs->auxservoctualspeed != rhs->auxservoctualspeed) {
    return false;
  }
  // auxservoactualtorque
  if (lhs->auxservoactualtorque != rhs->auxservoactualtorque) {
    return false;
  }
  // extpioinput
  for (size_t i = 0; i < 8; ++i) {
    if (lhs->extpioinput[i] != rhs->extpioinput[i]) {
      return false;
    }
  }
  // extpiooutput
  for (size_t i = 0; i < 8; ++i) {
    if (lhs->extpiooutput[i] != rhs->extpiooutput[i]) {
      return false;
    }
  }
  // extadcinput
  for (size_t i = 0; i < 4; ++i) {
    if (lhs->extadcinput[i] != rhs->extadcinput[i]) {
      return false;
    }
  }
  // extadcoutput
  for (size_t i = 0; i < 4; ++i) {
    if (lhs->extadcoutput[i] != rhs->extadcoutput[i]) {
      return false;
    }
  }
  // reconnect_flag
  if (lhs->reconnect_flag != rhs->reconnect_flag) {
    return false;
  }
  // exaxiscoordid
  if (lhs->exaxiscoordid != rhs->exaxiscoordid) {
    return false;
  }
  // slave_status_1
  if (lhs->slave_status_1 != rhs->slave_status_1) {
    return false;
  }
  // slave_status_2
  if (lhs->slave_status_2 != rhs->slave_status_2) {
    return false;
  }
  // slave_domain_id
  if (lhs->slave_domain_id != rhs->slave_domain_id) {
    return false;
  }
  // program_run_state
  if (lhs->program_run_state != rhs->program_run_state) {
    return false;
  }
  // speed_scale_manual
  if (lhs->speed_scale_manual != rhs->speed_scale_manual) {
    return false;
  }
  // speed_scale_auto
  if (lhs->speed_scale_auto != rhs->speed_scale_auto) {
    return false;
  }
  return true;
}
  ...
  fairino_msgs__msg__RobotNonrtState * output)
{
  ...
  // exaxistatus1
  for (size_t i = 0; i < 10; ++i) {
    output->exaxistatus1[i] = input->exaxistatus1[i];
  }
  // exaxispos2
  output->exaxispos2 = input->exaxispos2;
  // exaxistatus2
  for (size_t i = 0; i < 10; ++i) {
    output->exaxistatus2[i] = input->exaxistatus2[i];
  }
  // exaxispos3
  output->exaxispos3 = input->exaxispos3;
  // exaxistatus3
  for (size_t i = 0; i < 10; ++i) {
    output->exaxistatus3[i] = input->exaxistatus3[i];
  }
  // exaxispos4
  output->exaxispos4 = input->exaxispos4;
  // exaxistatus4
  for (size_t i = 0; i < 10; ++i) {
    output->exaxistatus4[i] = input->exaxistatus4[i];
  }
  // ft_fx_data
  ...
  // auxservoservoid
  output->auxservoservoid = input->auxservoservoid;
  // auxservoerrcode
  output->auxservoerrcode = input->auxservoerrcode;
  // auxservostate
  output->auxservostate = input->auxservostate;
  // auxservoactualpos
  output->auxservoactualpos = input->auxservoactualpos;
  // auxservoctualspeed
  output->auxservoctualspeed = input->auxservoctualspeed;
  // auxservoactualtorque
  output->auxservoactualtorque = input->auxservoactualtorque;
  // extpioinput
  for (size_t i = 0; i < 8; ++i) {
    output->extpioinput[i] = input->extpioinput[i];
  }
  // extpiooutput
  for (size_t i = 0; i < 8; ++i) {
    output->extpiooutput[i] = input->extpiooutput[i];
  }
  // extadcinput
  for (size_t i = 0; i < 4; ++i) {
    output->extadcinput[i] = input->extadcinput[i];
  }
  // extadcoutput
  for (size_t i = 0; i < 4; ++i) {
    output->extadcoutput[i] = input->extadcoutput[i];
  }
  // reconnect_flag
  output->reconnect_flag = input->reconnect_flag;
  // exaxiscoordid
  output->exaxiscoordid = input->exaxiscoordid;
  // slave_status_1
  output->slave_status_1 = input->slave_status_1;
  // slave_status_2
  output->slave_status_2 = input->slave_status_2;
  // slave_domain_id
  output->slave_domain_id = input->slave_domain_id;
  // program_run_state
  output->program_run_state = input->program_run_state;
  // speed_scale_manual
  output->speed_scale_manual = input->speed_scale_manual;
  // speed_scale_auto
  output->speed_scale_auto = input->speed_scale_auto;
  return true;
}
