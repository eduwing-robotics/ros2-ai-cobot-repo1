...
컨베이어 준비 실패의 `message`는 수동 정지, FAULT, 이동 중/상태 미확인, 제어 비활성, 비전 heartbeat 지연, 비전 미준비를 구분합니다. `MANUAL_STOP`은 준비 완료로 취급하지 않으며 제공 서버의 정지 해제 절차가 필요합니다. 시작 요청이나 상태 조회가 자동으로 reset하지 않습니다.
...
즉시 종료하며 장비 정지·복구 확인을 완료 조건으로 사용하지 않습니다. 취소 이후 영속적인 라인 잠금은 제공하지 않습니다.
이 분기는 관리자 인증이나 결정 이력을 추가하지 않으며 기존 버튼의 권한 체계를 따릅니다.

### 활성 작업 취소

`cancel`은 공정 단계와 장비 상태에 관계없이 일치하는 활성 Job을 Sequencer에서 즉시 종료합니다. 진행 Unit은 `FAILED`, Job은 `CANCELLED`로 같은 DB writer 경계에서 반영합니다. 장비 정지·복구·잔여 동작은 생산 취소 완료 조건이 아니며 운영자가 별도로 관리합니다. 동일 terminal Job 재요청은 기존 성공을 반환하고 DB 반영 실패만 취소 실패로 유지합니다.

`pause`와 `resume`은 기존 장비 상태 검증과 제어 확인 계약을 유지합니다. `cancel_reason`은 일치하는 활성 Job이면 비어 있으며 원격 capability·복구 상태와 무관합니다. 취소 후 Sequencer는 해당 실행 추적을 생산 흐름에서 해제하므로 지연된 설비 응답이 DB 완료나 다음 공정으로 이어지지 않습니다.
