...
| 품질 파일 | 구현 | 데이터시트 XLSX를 직접 읽고 Production FAIL 기록별 불량대책서 XLSX를 중복 없이 생성한다. |
...
- [x] 데이터시트를 DB에 적재하지 않고 XLSX에서 직접 읽는다.
...
- [x] 완료된 FAIL 기록별 불량대책서 XLSX를 생성하고 기존 파일은 덮어쓰지 않는다.
