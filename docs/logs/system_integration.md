...
## 2026-08-18 — TurtleBot 네트워크 주소 변경

- 최초 전달 내용을 FR5 주소 변경으로 잘못 기록했으나, 사용자 확인 후 바로
  정정했다.
- FR5 컨트롤러 IP는 기존 `192.168.58.2`를 유지한다.
- TurtleBot 컨베이어 IP를 `192.168.0.101`에서 `192.168.11.101`로 변경했다.
- 로컬 `config/ksmc.env`, 배포용 `config/ksmc.env.example`, 인수인계 문서를
  올바른 주소로 동기화했다.
- FR5 또는 TurtleBot에 연결하거나 이동 명령을 보내지는 않았다.

## 2026-08-18 — 컨베이어 전진 방향 반전

- 실제 컨베이어가 앞으로 움직이려면 TurtleBot 기준 후진이 필요함을 확인했다.
- 자동 정지 실행기의 논리적 벨트 전진을 `/cmd_vel`의 `linear.x=-0.10 m/s`로
  매핑했다.
- 제어 노드에 `--direction positive_x|negative_x` 옵션을 추가해 장비 방향을
  하드코딩된 음수 속도와 분리했다.
...
- build/install/vendor 산출물을 제외한 Shell 41개, Python 77개, JSON 55개,
  YAML 7개의 문법·파싱을 검사해 모두 통과했다.
- AI/Vision 패키지 전체에 ROS `ament_flake8`를 적용했다. 기존 import 순서와
  docstring 형식 문제 9개를 기능 변경 없이 정리했고 최종 16개 파일에서 오류가
  없음을 확인했다.
- Vision ROS 단위 테스트를 2중 정지선·복수 기판·간격 검증 테스트까지 20개로
  확장했으며 `20 passed`와 `colcon test-result: 0 errors, 0 failures`를 확인했다.
- `ros2_ws` 2개 패키지와 독립 `robot_ws` 4개 패키지를 각각 다시 빌드했다.
  `scripts/doctor.sh`도 `Failures: 0`으로 완료됐다.
- 실제 FR5, 그리퍼, TurtleBot 바퀴 또는 컨베이어에는 어떤 이동 명령도 보내지
  않았다. 소프트웨어 검증과 최종 장비에서의 실물 정지 검증을 구분해 기록한다.

## 2026-08-20 — PCB 운반 지그 Unity 통합

- `Cube.001.fbx`를 Unity의 `Assets/RobotArm/Fixture/`에 추가했다.
- FBX 외형 `85.30 x 124.82 mm`, 4핀 간격 약 `70.49 x 99.14 mm`를 사용해
  기판 8개 홀 중 안쪽 4개 홀 중심에 자동 정렬했다.
- FBX의 얇은 축을 Unity Y축으로 자동 변환하고 핀 패턴 중심을 기판 중심에
  맞췄다. 지그는 조립 부품이나 pick point로 집계하지 않아 기존 25개 부품 좌표를
  유지한다.
- Unity 6000.3.16f1 배치 빌드와 장면 저장, 25개 robot pick point export가
  성공했다. 완성품의 부러진 핀은 정렬 근거에서 제외하고 정상 FBX와 빈 기판의
  홀 배치를 기준으로 사용했다.
- 최초 축 변환에서 지그 핀이 아래를 향하고 손잡이가 기판 아래쪽을 향한 것을
  사용자 확인으로 발견했다. FBX 회전을 수정해 지그 본체는 기판 아래, 핀은
  Unity +Y 방향으로 기판 홀에 삽입되고, 손잡이는 기판 +Z(화면 위쪽) 방향으로
  돌출되도록 재배치했다. 장면 빌드 로그만으로 판단하지 않고 Unity +Z를 화면
  위로 고정한 정수직 탑뷰 PNG를 생성해, 손잡이가 위쪽에 있고 흰색 핀 4개가
  안쪽 4개 홀에 나타나는 것을 최종 확인했다.
- 사용자가 수동 배치한 장면에는 지그 Scale `X/Y=-549.26`, `Z=-10`의 비균일
  음수 스케일이 저장되어 있어 실측 치수와 핀 정렬을 신뢰할 수 없었다. 이를
  정상 실측 크기 `85.30 x 124.82 mm`로 복원했다.
- 지그 메시의 넓은 상단 지지면을 계산해 기판 바닥면과 맞췄으며 support gap은
  `0.0000 mm`이다. 정수직 렌더에서 홀/핀 외곽 중심을 재측정하고 X축을 총
  약 `1.98 mm` 보정해 최종 화면 오차를 약 `0.5 px`(`약 0.08 mm`)로 줄였다.
  핀 간격은 홀 간격과 일치하며 Unity 장면을 다시 저장했다.
- 이후 S22 최신 실물 프레임을 직접 캡처해 완성품 지그 손잡이가 영상 기준 기판
  상단 중앙 바깥쪽을 향하는 것을 확인하고 Unity 방향을 동일하게 유지했다.
- 핀 중심 검증을 1200px에서 2400px 정수직 탑뷰로 높여 재측정했다. 네 핀의
  홀 중심 차이는 최대 `1.5px`(모델 환산 약 `0.12mm`) 이하이고 평균 X 오차는
  약 `0.03mm`였다. 좌우 잔차 부호가 달라 추가 평행이동은 반대편 오차를
  증가시키므로 현재 위치를 모델 형상 기준 최종 정렬로 저장했다.
- 사용자 화면 기준 지그 손잡이가 남쪽에 남아 있다는 피드백에 따라 지그를
  Y축으로 180도 추가 회전해 요청한 북쪽 반대편으로 옮겼다. 회전 후 핀 중심을
  2400px 탑뷰로 다시 보정했으며 최종 잔차는 X 평균 `0.25px`, 최대 1px,
  Y 최대 1.5px(모델 환산 최대 약 `0.12mm`)였다. support gap은 `0.0000mm`를
  유지한다.
