# 과거 Real 모드 초기 Scene 동기화 계획

> 현재 구현 계획이 아니다. 당시 확인한 책임 경계와 미구현 제안을 보존한다.
...
- `VisionDetector.cs`,
  `Calibration.cs`,
  `CamMaster.cs`는 현재 Scene/Prefab에서 참조되지 않는다.
...
- `VisionDetector.cs`: batch snapshot 수신·검증·1회 적용
