...
```bash
~/KSMC/calibration/run_d435_rgb_stable.sh
```
...
source /opt/ros/jazzy/setup.bash
source ~/KSMC/robot_ws/install/setup.bash
python3 ~/KSMC/calibration/scripts/view_charuco_board.py
```
...
python3 ~/KSMC/calibration/scripts/capture_charuco_handeye_sample.py \
  --data-file ~/KSMC/calibration/data/handeye_refinement_samples.json
```
...
python3 ~/KSMC/calibration/scripts/capture_charuco_handeye_sample.py \
  --frames 20 \
  --data-file ~/KSMC/calibration/data/handeye_intrinsic33_samples.json
```
...
```bash
~/KSMC/calibration/run_marker_target_dry_run.sh \
  --marker-id 8 \
...
```bash
python3 ~/KSMC/calibration/scripts/validate_handeye_samples.py
```
...
```bash
~/KSMC/calibration/run_handeye_multipose_validation.sh \
  --marker-id 8 \
  --frames 20
```

기본 결과는
`~/KSMC/calibration/data/handeye_multipose_validation.csv`에 누적된다.
...
```bash
~/KSMC/calibration/run_intrinsic_capture.sh --label "01_center_front"
```
...
```bash
source /opt/ros/jazzy/setup.bash
python3 ~/KSMC/calibration/scripts/calibrate_charuco_intrinsics.py
```
