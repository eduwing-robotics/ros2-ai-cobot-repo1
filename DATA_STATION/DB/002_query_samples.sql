...
-- GET /alerts?status={status}&limit={limit}&offset={offset}
-- $1 NULL returns every status.
PREPARE datastation_alerts(text, integer, integer) AS
SELECT a.alert_code,
       a.part_id,
       p.part_name,
       a.defect_type,
       a.period_start,
       a.period_end,
       a.inspected_quantity,
       a.defective_quantity,
       a.defect_rate,
       a.threshold_rate,
       a.first_exceeded_at,
       a.source_recipe_version,
       a.alert_status,
       a.assignee,
       a.issued_at,
       a.initial_action_due_at,
       a.final_action_due_at,
       a.closed_at,
       a.document_path,
       c.containment_summary,
       c.root_cause_summary,
       c.escape_cause_summary,
       c.applied_recipe_version,
       c.verification_status,
       c.verified_defect_rate
FROM defect_report.alerts a
LEFT JOIN production.parts p ON p.part_id = a.part_id
LEFT JOIN defect_report.alert_countermeasures c ON c.alert_id = a.alert_id
WHERE $1 IS NULL OR a.alert_status = $1
ORDER BY a.issued_at DESC, a.alert_id DESC
LIMIT $2 OFFSET $3;

-- GET /parts/{part_id}
-- Datasheet candidates for a production part, resolved through part_group_links.
-- Returns no rows when the part has no datasheet group; the caller prints
-- "no datasheet" rather than failing.
PREPARE datastation_part(text) AS
WITH latest_supply AS (
    SELECT DISTINCT ON (candidate_id)
           ...
           unit_price,
           currency,
           price_quantity_basis,
           moq,
           stock_status,
           lead_time,
           price_checked_at
    FROM part_catalog.part_supply
    ORDER BY candidate_id, price_checked_at DESC NULLS LAST, supply_id DESC
)
SELECT l.part_id,
       g.group_id,
       g.category_label,
       ...
       cand.alternate_code,
       cand.manufacturer,
       cand.manufacturer_part_number,
       cand.key_spec,
       cand.lifecycle_status,
       cand.compatibility_status,
       cand.revalidation_items,
       cand.defect_relevance,
       cand.note,
       s.source_id,
       s.url,
       s.verified_content,
       s.checked_at,
       sup.supplier,
       sup.supplier_part_number,
       sup.unit_price,
       sup.currency,
       sup.price_quantity_basis,
       sup.moq,
       sup.stock_status,
       sup.lead_time,
       sup.price_checked_at,
       dl.source_file,
       dl.source_dated_on
FROM part_catalog.part_group_links l
JOIN part_catalog.part_groups g ON g.group_id = l.group_id
JOIN part_catalog.part_candidates cand ON cand.group_id = g.group_id
JOIN part_catalog.datasheet_loads dl ON dl.load_id = cand.load_id
LEFT JOIN part_catalog.sources s ON s.source_id = cand.source_id
LEFT JOIN latest_supply sup ON sup.candidate_id = cand.candidate_id
WHERE l.part_id = $1
ORDER BY CASE cand.candidate_role WHEN 'PRIMARY' THEN 0 ELSE 1 END,
         cand.alternate_code;

-- GET /parts/{part_id}/checklist
-- Quality checklist for the part's datasheet category, plus the shared rows.
PREPARE datastation_part_checklist(text) AS
SELECT q.category,
       q.incoming_inspection,
       q.assembly_control,
       q.reliability_test,
       q.action_on_anomaly
FROM part_catalog.quality_checklists q
WHERE q.category = '공통'
   OR q.category IN (
        SELECT g.category_label
        FROM part_catalog.part_group_links l
        JOIN part_catalog.part_groups g ON g.group_id = l.group_id
        WHERE l.part_id = $1
   )
ORDER BY CASE WHEN q.category = '공통' THEN 0 ELSE 1 END, q.category;
...
-- EXECUTE datastation_alerts(NULL, 50, 0);
-- EXECUTE datastation_part('CAP');
-- EXECUTE datastation_part_checklist('CAP');
